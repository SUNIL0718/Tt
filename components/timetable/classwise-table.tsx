...
export default function ClasswiseTable({ timetableId, slots }: Props) {
  const [data, setData] = useState<Record<string, any[]> | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`/api/timetable/classwise?timetableId=${timetableId}`)
      .then((res) => res.json())
      .then((json) => {
        if (json.success) {
          setData(json.data);
  ...
  if (!data || Object.keys(data).length === 0) {
    return (
      <div className="p-12 text-center bg-white rounded-3xl border border-dashed border-slate-200">
        <p className="text-slate-500 font-bold">No class-wise data available.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      {Object.entries(data).map(([className, entries]) => (
        <div key={className} className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="px-5 py-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
            <h2 className="text-lg font-black text-slate-900 tracking-tight">Class {className}</h2>
          </div>
                          ...
                          <td key={`${day}-${periodIndex}`} className="border border-slate-200 p-2 align-top">
                            {cellData ? (
                              <div className="flex flex-col gap-1 min-h-[50px] justify-center items-center">
                                <span className="font-bold text-[11px] text-slate-800 leading-tight block">{cellData.subject}</span>
                                {cellData.teacher && (
                                  <span className="text-[9px] font-semibold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded-md">
                                    {cellData.teacher}
                                  </span>
                                )}
