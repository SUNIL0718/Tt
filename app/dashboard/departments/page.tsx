...
import AddDepartmentModal from "@/components/timetable/add-department-modal";
...
export default async function DepartmentsPage() {
  ...
  return (
    <div className="space-y-6">
      <GenericHeader 
        title="Departments" 
        subtitle="Manage your institution's departments" 
          ...
          <tbody>
            {departments.length === 0 ? (
              ...
              departments.map((dept: any) => (
                <tr key={dept._id.toString()} className="border-b bg-white hover:bg-gray-50">
                  <td className="px-6 py-4 font-bold text-indigo-600 uppercase tracking-widest">{dept.code}</td>
                  <td className="px-6 py-4 font-medium text-gray-900">{dept.name}</td>
                  <td className="px-6 py-4">
                     <form action={deleteServerAction.bind(null, dept._id.toString())}>
                        <button className="text-red-600 hover:underline font-bold">Delete</button>
                     </form>
                  </td>
                </tr>
              ))
