...
import AddSubjectModal from "@/components/timetable/add-subject-modal";
...
export default async function SubjectsPage() {
  ...
  const subjects = await Subject.find(filter).populate({ path: "departmentId", strictPopulate: false }).lean();
  ...
  return (
    <div className="space-y-6">
      <GenericHeader 
        title="Subjects" 
        subtitle="Manage your curriculum and academic courses" 
        entityType="SUBJECT" 
        AddModal={AddSubjectModal}
          ...
          <tbody>
            {subjects.length === 0 ? (
              ...
              subjects.map((sub: any) => (
                <tr key={sub._id.toString()} className="border-b bg-white hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                     <form action={deleteSubjectAction.bind(null, sub._id.toString())}>
